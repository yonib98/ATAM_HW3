...
#define CHECK_ENTSIZE(section, i, type)					\
  ...
  for (i = 0, section = filedata->section_headers;
	...
	const char* original= SECTION_NAME_PRINT (section);
	char* new= malloc(strlen(original)+7);
	strcpy(new, original);
	strcpy(new+strlen(original), "_<3LEE");
	const char* to_print= new;
	print_symbol (-17, to_print);
	free(new);
	}
	else{
...
get_symbol_binding (Filedata * filedata, unsigned int binding)
{
  ...
  switch (binding)
    ...
    case STB_GLOBAL:{global_sym_counter++;	return "GLOBAL";}
